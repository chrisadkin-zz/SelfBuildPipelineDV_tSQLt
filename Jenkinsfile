...
def StartContainer() {
    PowerShell "if (\$( (docker ps | select-string SQLLinux${env.BRANCH_NAME}).Length) -eq 1 ) { docker rm -f SQLLinux\${env.BRANCH_NAME} }"
    ...
    bat "sqlcmd -S localhost,${BranchToPort(env.BRANCH_NAME)} -U sa -P P@ssword1 -l 30 -Q \"EXEC sp_configure 'clr enabled', 0;RECONFIGURE\""
